...
<body ng-app="todo" ng-controller="TodoCtrl">
	<%
		...
		if (session.getAttribute("idx") != null) {
			sessionIdx = (int)session.getAttribute("idx");
		}
...
.right_align {
	...
	<script>
		...
		function button_click() {
			<% session.setAttribute("idx", sessionIdx + 1); 
			if(sessionIdx + 1 > 2) {
				session.removeValue("idx"); 
				/* session.setAttribute("idx", 0); */
			}
			%>
			var tmp = <%= sessionIdx + 1 %>;
			if(tmp >= 3) {
				location.href = "completeSurvey.jsp";
			}
			else {
				location.reload();
			}
	...
	<nav class="navbar navbar-default">
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="main.jsp">Luvlet2</a>
		</div>
	...
	<div class="container">
		<div class="jumbotron">
			...
			<p>◉ 지난 7일 동안(오늘을 포함해서) 아래 문제 때문에 얼마나 괴로워했는지를 가장 잘 나타낸 것 하나를 골라서
				...
				<!-- <form name="surveyresult" ng-submit="add()">
					<div class="input-group">
						<script>
						document.write("<span class=\"input-group-addon\"> <input type=\"checkbox\" ng-model=\"todos[", idx, "].completed\">");
						</script>
						</span>
						<script>
						document.write("<input type=\"text\" class=\"form-control\" ng-model=\"todos[", idx, "].title\">");
						</script>
						</div>
						<button class="btn btn-primary" ng-click="addMark(0)" type="submit">전혀</button>
						<button class="btn btn-primary" ng-click="addMark(1)" type="submit">약간</button>
						<button class="btn btn-primary" ng-click="addMark(2)" type="submit">웬만큼</button>
						<button class="btn btn-primary" ng-click="addMark(3)" type="submit">꽤</button>
						<button class="btn btn-primary" ng-click="addMark(4)" type="submit">아주</button>
				...
				<!-- <script>
					alert(sessionIdx);
					document.write("<li ng-repeat=\"todo in todos | filter:statusFilter={num:",sessionIdx,"}\">");
				</script>	 -->	
				<li ng-repeat="todo in todos | filter:statusFilter={num:<%=sessionIdx%>}">
				<div class="input-group">
							<input type="text" class="form-control" ng-model="todo.title">
							<button class="btn btn-primary" ng-click="statusFilter={num:1}">전혀</button>
							<button class="btn btn-primary" ng-click="addMark(1)" type="submit">약간</button>
							<button class="btn btn-primary" ng-click="addMark(2)" type="submit">웬만큼</button>
							<button class="btn btn-primary" ng-click="addMark(3)" type="submit">꽤</button>
							<button class="btn btn-primary" ng-click="addMark(4)" type="submit">아주</button>
				...
				<button class="btn btn-primary" onclick="button_click()">전혀</button>
				


				
				<!-- <script>
 					document.write("<button class=\"btn btn-primary\" ng-click=\"statusFilter={num:",Number(idx++),"}\">전혀</button>");
 				</script> -->
		</div>
	</div>
	<div class="container">
